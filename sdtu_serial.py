...
#
import usr.sdtu_functions as functions
import usr.const as Const
...
class Serial:
    def __init__(self, uart_id, baudrate=9600, data_bits=8, parity=None,stop_bits=1,  flow_ctl=0, ctrl_pin=None):
        self._uart = UART(uart_id, baudrate, data_bits, parity, stop_bits, flow_ctl)
        print(self._uart)
        if ctrl_pin is not None:
            self._ctrlPin = Pin(ctrl_pin, Pin.OUT)
        else:
            self._ctrlPin = None
        # self.char_time_ms 是1个字符时间
    ...
    def _uart_read(self):
        ...
        for x in range(1, 40):
            msgLen = self._uart.any()
            if msgLen:
                response.extend(self._uart.read(msgLen))
